...
class FlowNetSDeconv(tf.keras.Model):
    ...
    def call(self, inputs, conv_outputs):
        ...
        flow2_upsampled = tf.image.resize(flow2, size=(x.shape[1], x.shape[2]), method='bilinear')
...
def get_encoder(latent_dim = 64):
    ...
    inputs = tf.concat([encoder_inputs, encoder_inputs], axis=-1)  # 複製輸入
    encode = FlowNetSConv()
    conv_outputs = encode(inputs)

    # 打印 conv_outputs 的長度和內容，幫助調試
...
def show_subplot(original, reconstructed):
    plt.subplot(1, 2, 1)
    plt.imshow(original.squeeze() + 0.5, cmap='gray')  # 確保是灰度圖像
    plt.title('original')
    plt.axis('off')
    ...
    if reconstructed.shape[-1] == 2:
        # 如果有兩個通道，選擇第一個通道顯示
        plt.imshow(reconstructed[..., 0].squeeze() + 0.5, cmap='gray')
    else:
