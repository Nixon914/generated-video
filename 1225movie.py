...
class CorrelationLayer(tf.keras.layers.Layer):
    ...
    def call(self, input_1, input_2):
        ...
        corr_tensors = []
        #計算滑動窗口的相關性，最大偏移量為max_displacement
...
def show_subplot(original, reconstructed):
    plt.subplot(1, 2, 1)
    plt.imshow(original.squeeze() + 0.5) # squeeze為將維度1的shape用掉
    plt.title('original')
    plt.axis('off')
...
image1 = tf.random.normal([1, 384, 512, 3]) 
image2 = tf.random.normal([1, 384, 512, 3])
