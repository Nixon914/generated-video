...
def separable_conv(batch_norm, in_channels, out_channels, kernel_size=3, stride=1):
    layers_list = []
...
class FlowNetSConv(tf.keras.Model):
    def __init__(self, batch_norm=True):
        ...
        self.conv1 = separable_conv(self.batch_norm, 6, 64, kernel_size=7, stride=2)
        self.conv2 = separable_conv(self.batch_norm, 64, 128, kernel_size=5, stride=2)
        self.conv3 = separable_conv(self.batch_norm, 128, 256, kernel_size=5, stride=2)
        self.conv_redir = separable_conv(self.batch_norm, 256, 32, kernel_size=1, stride=1)
        self.corr = CorrelationLayer(max_displacement=10)
        self.conv3_1 = separable_conv(self.batch_norm, 256, 256)
        self.conv4 = separable_conv(self.batch_norm, 256, 512, stride=2)
        self.conv4_1 = separable_conv(self.batch_norm, 512, 512)
        self.conv5 = separable_conv(self.batch_norm, 512, 512, stride=2)
        self.conv5_1 = separable_conv(self.batch_norm, 512, 512)
        self.conv6 = separable_conv(self.batch_norm, 512, 1024, stride=2)
        self.conv6_1 = separable_conv(self.batch_norm, 1024, 1024)
...
class VQVAETrainer(keras.models.Model):
    def __init__(self, train_variance, latent_dim = 32, num_embedding = 128,l1_factor=0.01, l2_factor=0.01, **kwargs):
        super().__init__(**kwargs)
    ...
    def train_step(self, x): # 程式中沒被呼叫到，但是在fit中會被使用
        with tf.GradientTape() as tape: # 用來記錄前向傳播中的操作
            ...
            l1_loss = 0
            for weight in self.vqvae.trainable_weights:
                if 'kernel' in weight.name or 'embedding' in weight.name:
                    l1_loss += tf.reduce_sum(tf.abs(weight))
            l1_loss *= self.l1_factor
        ...
        return{
            ...
            'rc' : self.reconstruction_loss_tracker.result(),
            'vq' : self.vq_loss_tracker.result(),
            'l1': self.l1_loss_tracker.result(),
            'l2': self.l2_loss_tracker.result(),
        }
...
def show_subplot(original, reconstructed):
    plt.subplot(1, 2, 1)
    plt.imshow(original + 0.5)  # 移除 squeeze() 因為CIFAR10是彩色圖像
    plt.title('original')
    plt.axis('off')
    ...
    if reconstructed.shape[-1] == 2:
        # 如果有兩個通道，使用第一個通道
        plt.imshow(reconstructed[..., 0] + 0.5)
    else:
        plt.imshow(reconstructed + 0.5)
...
reconstruction_test = trained_vqvae_model.predict(test_images)

for test_image, reconstruction_image in zip(test_images, reconstruction_test):
    show_subplot(test_image, reconstruction_image)