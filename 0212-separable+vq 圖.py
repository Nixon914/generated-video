...
class VQVAETrainer(keras.models.Model):
    ...
    def calculate_similarity(self, original, generated):
        ...
        similarity = 1.0 - tf.minimum(mse / self.train_variance, 1.0)
...
class ReconstructionCallback(keras.callbacks.Callback):
    def __init__(self, test_images, num_samples=5):
        super(ReconstructionCallback, self).__init__()
        self.test_images = test_images
        self.num_samples = min(num_samples, len(test_images))
    
    def on_epoch_end(self, epoch, logs=None):
        # 選擇固定的測試圖像進行重建
        test_samples = self.test_images[:self.num_samples]
        reconstructions = self.model.vqvae.predict(test_samples)
        
        # 創建一個新的圖形
        plt.figure(figsize=(15, 3))
        
        # 為每個樣本創建兩個子圖（原始圖像和重建圖像）
        for i in range(self.num_samples):
            # 原始圖像
            plt.subplot(2, self.num_samples, i + 1)
            plt.imshow(test_samples[i] + 0.5)  # 還原標準化
            plt.axis('off')
            if i == 0:
                plt.title('Original')
            ...
            plt.imshow(reconstructions[i] + 0.5)  # 還原標準化
            plt.axis('off')
            if i == 0:
...
# 2. 準備回調函數所需的測試圖像
num_test_samples = 5
test_indices = np.random.choice(len(x_test_scaled), num_test_samples, replace=False)
test_images_for_viz = x_test_scaled[test_indices]

# 3. 創建回調函數實例
reconstruction_callback = ReconstructionCallback(test_images_for_viz)
